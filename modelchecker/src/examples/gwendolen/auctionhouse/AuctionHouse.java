...
import gwendolen.auctionhouse.model.Auction;
import gwendolen.auctionhouse.model.Bid;
import gwendolen.auctionhouse.model.Customer;
...
public class AuctionHouse extends DefaultEnvironment implements MCAPLJobber {
	...
	private Unifier addToAuction(int id, String agent) throws AILexception {
		if (customers.containsKey(agent) && auctions.containsKey(id) && !auctions.get(id).participates(agent)) {
			...
			throw new AILexception("participate error: reg: "+ customers.containsKey(agent) + ", auction: "+ auctions.containsKey(id) +", not participate:"+ !auctions.get(id).participates(agent));
		}
	}
	...
	private Unifier removeFromAuction(int id, String agent) throws AILexception {
		if (auctions.containsKey(id) && auctions.get(id).participates(agent)) {
			...
			throw new AILexception("leave error: auction: "+ auctions.containsKey(id) +", participate:"+ auctions.get(id).participates(agent));
		}
	}
	
	private Literal createParticipantPercept(int id, String ag) {
		Literal percept = new Literal("participant");
	...
	private Literal createPaidPercept(int id) {
		Literal percept = new Literal("paid");
		percept.addTerm(new NumberTermImpl(id));
		return percept;
	}
